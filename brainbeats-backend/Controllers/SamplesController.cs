  ...
  public class SamplesController : ControllerBase {
    ...
    ReadSample(Sample sample) {
      ...
      FeedIterator<Sample>feedIterator =
          db.GetContainer("Samples").GetItemQueryIterator<Sample>(
              queryDefinition);
      ...
      while (feedIterator.HasMoreResults) {
        ...
        foreach(Sample s in response) { samples.Add(s); }
      }
    ...
    UpdateSample(Sample sample) {
      ...
      IList<PropertyInfo>properties =
          new List<PropertyInfo>(updatedSample.GetType().GetProperties());
      ...
      ItemResponse<Sample>res =
          await db.GetContainer("Samples").ReplaceItemAsync(
              updatedSample, updatedSample.id,
              new PartitionKey(updatedSample.id));
    ...
    DeleteSample(Sample sample) {
      ...
      try {
        ItemResponse<Sample>res =
            await db.GetContainer("Samples").DeleteItemAsync<Sample>(
                sample.id, new PartitionKey(sample.id));
