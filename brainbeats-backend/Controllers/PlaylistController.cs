...
namespace brainbeats_backend.Controllers {
  ...
  public class PlaylistController : ControllerBase {
    ...
    CreatePlaylist(dynamic req) {
      ...
      string queryString1 =
          "g.addV('playlist')" + ".property('type', 'playlist')" +
          $".property('id', '{playlistId}')" + $".property('name', '{name}')" +
      ...
      if (beatId != null) {
        string addEdge = $".addE('CONTAINS')" + $".to(g.V('{beatId}'))";
    ...
    ReadPlaylist(dynamic req) {
      ...
      try {
        var result =
            await DatabaseConnection.Instance.ExecuteQuery(queryString);
        return Ok(result);
      } catch {
        return BadRequest();
      }
    ...
    UpdatePlaylist(dynamic req) {
      ...
      if (playlistId == null || (name == null && image == null &&
                                 isPrivate == null && isDeleted == null)) {
        return BadRequest("Malformed Request");
      }
      ...
      try {
        var result =
            await DatabaseConnection.Instance.ExecuteQuery(sb.ToString());
        return Ok(result);
      } catch {
        return BadRequest();
      }
    ...
    public async Task<IActionResult>
    UpdatePlaylistDeleteBeats(dynamic req) {
      var body = JsonConvert.DeserializeObject<dynamic>(req.ToString());
    ...
    [Route("updateAddBeat")]
    public async Task<IActionResult>
    UpdatePlaylistAddBeats(dynamic req) {
      var body = JsonConvert.DeserializeObject<dynamic>(req.ToString());
